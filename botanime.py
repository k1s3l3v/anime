...
def evaluate(model_name, chars, idx, start_text=' ', prediction_len=200, temp=0.3):
    hidden = model_name.init_hidden()
    idx_input = [chars[char] for char in start_text]
    train = torch.LongTensor(idx_input).view(-1, 1, 1).to(device)
    predicted_text = start_text

    _, hidden = model_name(train, hidden)

    inp = train[-1].view(-1, 1, 1)

    for k in range(prediction_len):
        output, hidden = model_name(inp.to(device), hidden)
        output_logits = output.cpu().data.view(-1)
        p_next = functional.softmax(output_logits / temp, dim=-1).detach().cpu().data.numpy()
        top_index = np.random.choice(len(chars), p=p_next)
        inp = torch.LongTensor([top_index]).view(-1, 1, 1).to(device)
        predicted_char = idx[top_index]
...
def start_message(message):
    bot.send_message(message.chat.id, "Здравствуйте, Вы попали на anime fanpage!\nЧтобы создать свой комикс, введите "
                                      "/generatemycomics\nЧтобы посмотреть на случайный комикс, "
                                      "введите /generatecomics\nЧтобы посмотреть на случайно составленный текст, "
                                      "основанный на аниме субтитрах, введите /generatetext", reply_markup=keyboard)
...
def get_list_genres(genre):
    result = dic[genre[0]]
    for genre in genre[1:]:
